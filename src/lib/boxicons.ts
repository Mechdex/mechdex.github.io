const regularIcons = [
    "caret-up-circle",
    "note",
    "dice-5",
    "bus-school",
    "bible",
    "chevron-down-square",
    "arrow-from-bottom",
    "wifi-2",
    "book-reader",
    "message-rounded-check",
    "message-minus",
    "message-rounded-edit",
    "caret-right-circle",
    "category-alt",
    "comment-minus",
    "vector",
    "pointer",
    "movie-play",
    "message-edit",
    "calendar-event",
    "envelope-open",
    "game",
    "font-family",
    "caret-up",
    "grid-small",
    "message-x",
    "abacus",
    "microchip",
    "message-rounded-detail",
    "rotate-left",
    "coin-stack",
    "dice-3",
    "task-x",
    "home-smile",
    "chevron-left-circle",
    "cctv",
    "card",
    "wifi-1",
    "baseball",
    "message-alt-detail",
    "shield-x",
    "recycle",
    "border-inner",
    "arrow-from-right",
    "magnet",
    "caret-up-square",
    "bookmark-alt-plus",
    "badge",
    "intersect",
    "caret-left-square",
    "layer-plus",
    "caret-left",
    "chevron-up-square",
    "bookmark-alt",
    "bookmark-alt-minus",
    "unlink",
    "show-alt",
    "camera-movie",
    "unite",
    "arch",
    "chevron-down-circle",
    "heart-square",
    "arrow-to-left",
    "wifi-0",
    "current-location",
    "coin",
    "sort-up",
    "comment-detail",
    "message-alt-minus",
    "library",
    "move-vertical",
    "loader",
    "reply",
    "sitemap",
    "strikethrough",
    "tag",
    "crop",
    "globe",
    "command",
    "share-alt",
    "file-blank",
    "station",
    "radar",
    "cart",
    "server",
    "highlight",
    "chip",
    "notification",
    "phone-outgoing",
    "dollar-circle",
    "menu-alt-right",
    "circle",
    "horizontal-center",
    "user-circle",
    "trash",
    "list-minus",
    "barcode",
    "repost",
    "down-arrow",
    "store",
    "mobile-alt",
    "bus",
    "cog",
    "dislike",
    "tennis-ball",
    "cloud-lightning",
    "table",
    "dialpad",
    "support",
    "left-arrow",
    "slider-alt",
    "log-out",
    "cloud-light-rain",
    "sidebar",
    "laptop",
    "message-rounded",
    "disc",
    "carousel",
    "first-aid",
    "football",
    "braille",
    "bomb",
    "chevron-right-square",
    "message-square-error",
    "message-alt-error",
    "brain",
    "message-square-check",
    "caret-down-square",
    "color-fill",
    "test-tube",
    "stats",
    "id-card",
    "been-here",
    "message-alt-x",
    "church",
    "arrow-from-top",
    "cabinet",
    "message-square-edit",
    "expand-alt",
    "calendar-edit",
    "door-open",
    "capsule",
    "help-circle",
    "location-plus",
    "comment-error",
    "caret-down",
    "message-alt-add",
    "exclude",
    "beer",
    "bookmark-heart",
    "window-alt",
    "outline",
    "message-square-detail",
    "list-ul",
    "rename",
    "analyse",
    "bath",
    "grid-alt",
    "tv",
    "poll",
    "user-minus",
    "rocket",
    "film",
    "star",
    "wifi",
    "alarm-off",
    "lock",
    "dumbbell",
    "paperclip",
    "underline",
    "columns",
    "dock-right",
    "adjust",
    "body",
    "drink",
    "glasses-alt",
    "home-heart",
    "collection",
    "ruler",
    "duplicate",
    "link",
    "up-arrow",
    "video-plus",
    "plug",
    "aperture",
    "list-ol",
    "window-open",
    "taxi",
    "tab",
    "area",
    "shopping-bag",
    "voicemail",
    "water",
    "move",
    "map",
    "dock-left",
    "collapse",
    "code-block",
    "notification-off",
    "layer",
    "task",
    "pin",
    "log-in",
    "pie-chart-alt",
    "font-color",
    "shape-polygon",
    "comment-x",
    "bong",
    "cookie",
    "chevron-up-circle",
    "trip",
    "message-rounded-error",
    "dice-2",
    "arrow-to-bottom",
    "traffic-cone",
    "message-rounded-add",
    "donate-heart",
    "badge-check",
    "home-circle",
    "webcam",
    "caret-left-circle",
    "wine",
    "dice-4",
    "border-none",
    "info-square",
    "comment-edit",
    "trim",
    "chair",
    "refresh",
    "blanket",
    "tag-alt",
    "arrow-to-right",
    "atom",
    "paste",
    "link-external",
    "list-plus",
    "heading",
    "calculator",
    "trash-alt",
    "detail",
    "window",
    "closet",
    "swim",
    "user",
    "link-alt",
    "shield-quarter",
    "box",
    "user-check",
    "paper-plane",
    "cube",
    "cart-alt",
    "x",
    "phone",
    "slider",
    "anchor",
    "alarm",
    "message",
    "power-off",
    "share",
    "joystick",
    "heart",
    "like",
    "shield",
    "trophy",
    "export",
    "sun",
    "edit",
    "layout",
    "folder-open",
    "phone-incoming",
    "group",
    "list-check",
    "book-heart",
    "merge",
    "paint",
    "calendar-heart",
    "heart-circle",
    "barcode-reader",
    "chevron-left-square",
    "arrow-from-left",
    "message-check",
    "glasses",
    "sticker",
    "message-square-add",
    "walk",
    "caret-right",
    "message-add",
    "brush",
    "message-detail",
    "line-chart-down",
    "message-error",
    "layer-minus",
    "calendar-exclamation",
    "alarm-snooze",
    "save",
    "usb",
    "calendar-alt",
    "phone-call",
    "map-alt",
    "comment-add",
    "message-rounded-minus",
    "equalizer",
    "low-vision",
    "credit-card-front",
    "move-horizontal",
    "coffee-togo",
    "eraser",
    "sort-alt-2",
    "bracket",
    "message-alt-check",
    "arrow-to-top",
    "category",
    "dice-1",
    "network-chart",
    "grid-horizontal",
    "joystick-button",
    "book-add",
    "alarm-exclamation",
    "message-alt-edit",
    "message-square-minus",
    "bone",
    "timer",
    "meh",
    "loader-alt",
    "infinite",
    "lock-open",
    "dock-bottom",
    "hdd",
    "check-circle",
    "headphone",
    "font",
    "windows",
    "hourglass",
    "battery",
    "redo",
    "file",
    "package",
    "fingerprint",
    "undo",
    "mobile",
    "world",
    "hash",
    "wind",
    "right-arrow",
    "globe-alt",
    "navigation",
    "bot",
    "select-multiple",
    "news",
    "spray-can",
    "medal",
    "message-rounded-x",
    "vial",
    "mask",
    "book-alt",
    "camera-home",
    "sort-down",
    "caret-right-square",
    "tachometer",
    "rewind-circle",
    "comment-check",
    "money",
    "message-square-x",
    "minus-back",
    "chevron-right-circle",
    "donate-blood",
    "caret-down-circle",
    "dice-6",
    "scan",
    "calendar-week",
    "calendar-star",
    "repeat",
    "meteor",
    "grid-vertical",
    "minus-front",
    "border-outer",
    "transfer",
    "bed",
    "user-plus",
    "skip-next-circle",
    "briefcase-alt",
    "award",
    "loader-circle",
    "images",
    "target-lock",
    "window-close",
    "car",
    "handicap",
    "purchase-tag",
    "reply-all",
    "git-compare",
    "subdirectory-left",
    "train",
    "subdirectory-right",
    "italic",
    "pie-chart",
    "cut",
    "import",
    "cloud-rain",
    "git-merge",
    "filter",
    "pencil",
    "text",
    "movie",
    "cloud-drizzle",
    "user-x",
    "upload",
    "pen",
    "plus",
    "ball",
    "skip-previous-circle",
    "font-size",
    "git-pull-request",
    "dock-top",
    "home",
    "credit-card",
    "home-alt",
    "search-alt",
    "certification",
    "revision",
    "flag",
    "cast",
    "joystick-alt",
    "spreadsheet",
    "microphone-off",
    "crown",
    "wallet",
    "check-double",
    "planet",
    "restaurant",
    "expand",
    "music",
    "hotel",
    "git-commit",
    "minus",
    "chalkboard",
    "paragraph",
    "sort",
    "shield-alt",
    "history",
    "git-repo-forked",
    "photo-album",
    "wrench",
    "album",
    "pulse",
    "rotate-right",
    "cloud-snow",
    "check",
    "upvote",
    "video-off",
    "happy",
    "shuffle",
    "mouse",
    "selection",
    "screenshot",
    "git-branch",
    "menu-alt-left",
    "bold",
    "downvote",
    "sad",
    "send",
    "dizzy",
    "rewind",
    "up-arrow-circle",
    "bullseye",
    "basketball",
    "accessibility",
    "female",
    "bell-off",
    "street-view",
    "crosshair",
    "smile",
    "landscape",
    "calendar-minus",
    "happy-heart-eyes",
    "dish",
    "right-indent",
    "shocked",
    "fullscreen",
    "wifi-off",
    "speaker",
    "gift",
    "shekel",
    "chevrons-down",
    "registered",
    "collapse-vertical",
    "toggle-left",
    "left-top-arrow-circle",
    "compass",
    "dna",
    "ruble",
    "droplet",
    "gas-pump",
    "bolt-circle",
    "play",
    "tone",
    "phone-off",
    "dots-vertical",
    "checkbox-square",
    "sort-z-a",
    "show",
    "meh-blank",
    "happy-beaming",
    "filter-alt",
    "border-bottom",
    "bar-chart-alt-2",
    "run",
    "cool",
    "square",
    "chevrons-left",
    "camera-off",
    "minus-circle",
    "chevrons-right",
    "zoom-in",
    "calendar-x",
    "customize",
    "mouse-alt",
    "collapse-horizontal",
    "grid",
    "vertical-center",
    "time",
    "chevron-up",
    "transfer-alt",
    "align-left",
    "time-five",
    "left-arrow-circle",
    "bug-alt",
    "data",
    "image-alt",
    "microphone",
    "checkbox",
    "calendar-check",
    "border-top",
    "question-mark",
    "exit-fullscreen",
    "book-content",
    "shape-square",
    "shape-circle",
    "euro",
    "basket",
    "dollar",
    "plus-circle",
    "memory-card",
    "play-circle",
    "log-out-circle",
    "cloud-download",
    "skip-next",
    "bitcoin",
    "sort-a-z",
    "briefcase",
    "right-top-arrow-circle",
    "left-arrow-alt",
    "bell-plus",
    "angry",
    "printer",
    "dots-horizontal",
    "terminal",
    "book-bookmark",
    "briefcase-alt-2",
    "fridge",
    "buildings",
    "video-recording",
    "math",
    "male-female",
    "popsicle",
    "circle-half",
    "home-alt-2",
    "cheese",
    "circle-quarter",
    "baguette",
    "bowl-rice",
    "pie-chart-alt-2",
    "wallet-alt",
    "reset",
    "dots-vertical-rounded",
    "mobile-landscape",
    "toggle-right",
    "message-alt",
    "bookmark-minus",
    "border-right",
    "tired",
    "envelope",
    "key",
    "border-all",
    "radio-circle",
    "captions",
    "slideshow",
    "cycling",
    "hive",
    "video",
    "upside-down",
    "mobile-vibration",
    "bar-chart-square",
    "clinic",
    "square-rounded",
    "arrow-back",
    "image-add",
    "checkbox-checked",
    "download",
    "bar-chart-alt",
    "lock-open-alt",
    "message-rounded-dots",
    "chevron-left",
    "copy-alt",
    "bar-chart",
    "bluetooth",
    "signal-1",
    "no-signal",
    "reflect-vertical",
    "cart-add",
    "building",
    "paint-roll",
    "extension",
    "archive",
    "expand-horizontal",
    "bowl-hot",
    "lemon",
    "qr-scan",
    "signal-4",
    "signal-2",
    "signal-3",
    "signal-5",
    "desktop",
    "code-alt",
    "pound",
    "right-arrow-alt",
    "comment-dots",
    "comment",
    "credit-card-alt",
    "archive-in",
    "border-left",
    "rectangle",
    "stop-circle",
    "ghost",
    "volume-full",
    "calendar",
    "moon",
    "check-shield",
    "first-page",
    "folder-plus",
    "book",
    "laugh",
    "left-indent",
    "plus-medical",
    "edit-alt",
    "cylinder",
    "cube-alt",
    "align-right",
    "checkbox-minus",
    "leaf",
    "trending-up",
    "brightness",
    "dots-horizontal-rounded",
    "yen",
    "add-to-queue",
    "bookmarks",
    "candles",
    "cross",
    "circle-three-quarter",
    "fork",
    "lira",
    "x-circle",
    "bell",
    "hide",
    "notepad",
    "down-arrow-circle",
    "volume-low",
    "space-bar",
    "up-arrow-alt",
    "qr",
    "directions",
    "cuboid",
    "buoy",
    "fast-forward",
    "copyright",
    "female-sign",
    "file-find",
    "skip-previous",
    "message-alt-dots",
    "health",
    "exit",
    "chart",
    "copy",
    "message-dots",
    "book-open",
    "clipboard",
    "polygon",
    "calendar-plus",
    "last-page",
    "coffee",
    "archive-out",
    "bulb",
    "bookmark",
    "cloud-upload",
    "male",
    "mail-send",
    "chat",
    "podcast",
    "party",
    "money-withdraw",
    "hard-hat",
    "reflect-horizontal",
    "bell-minus",
    "lock-alt",
    "doughnut-chart",
    "wink-smile",
    "align-middle",
    "cloud",
    "label",
    "band-aid",
    "food-tag",
    "user-voice",
    "camera",
    "log-in-circle",
    "cake",
    "volume-mute",
    "male-sign",
    "check-square",
    "rupee",
    "at",
    "map-pin",
    "zoom-out",
    "stopwatch",
    "folder-minus",
    "purchase-tag-alt",
    "shape-triangle",
    "right-down-arrow-circle",
    "brightness-half",
    "message-square-dots",
    "receipt",
    "collapse-alt",
    "chevron-right",
    "stop",
    "devices",
    "block",
] as const;

const solidIcons = [
    "search",
    "dice-1",
    "camera-home",
    "comment-edit",
    "coffee-alt",
    "radiation",
    "piano",
    "home-smile",
    "arrow-from-right",
    "bookmark-alt-plus",
    "layer-minus",
    "flame",
    "car-mechanic",
    "chevrons-up",
    "caret-left-square",
    "beer",
    "church",
    "chevrons-left",
    "virus",
    "message-error",
    "comment-check",
    "thermometer",
    "directions",
    "arrow-to-top",
    "pointer",
    "cart-add",
    "cabinet",
    "home-heart",
    "message-alt-detail",
    "credit-card-front",
    "keyboard",
    "shapes",
    "layer-plus",
    "copyright",
    "chevron-down",
    "capsule",
    "sticker",
    "chevron-left",
    "cloud-lightning",
    "bone",
    "vector",
    "dice-4",
    "gas-pump",
    "message-square-error",
    "message-rounded-detail",
    "coin-stack",
    "guitar-amp",
    "user-rectangle",
    "bullseye",
    "comment-minus",
    "chevron-left-square",
    "left-arrow-square",
    "chess",
    "dice-5",
    "movie-play",
    "up-arrow-square",
    "message-square-add",
    "basketball",
    "message-square-minus",
    "binoculars",
    "low-vision",
    "info-square",
    "dice-2",
    "diamond",
    "book-alt",
    "eraser",
    "location-plus",
    "face-mask",
    "chevron-down-circle",
    "dryer",
    "category",
    "pencil",
    "like",
    "folder",
    "wrench",
    "music",
    "inbox",
    "quote-right",
    "zap",
    "plug",
    "skull",
    "planet",
    "user-circle",
    "adjust",
    "cloud",
    "chart",
    "terminal",
    "message-rounded",
    "x-square",
    "select-multiple",
    "lock",
    "phone-incoming",
    "crown",
    "send",
    "volume",
    "paste",
    "folder-open",
    "book",
    "rename",
    "shopping-bag-alt",
    "up-arrow-circle",
    "watch",
    "home",
    "error",
    "filter-alt",
    "shield",
    "cookie",
    "message-detail",
    "game",
    "shield-x",
    "caret-up-square",
    "virus-block",
    "tree",
    "arch",
    "ev-station",
    "bookmark-alt",
    "chevron-right-circle",
    "coffee-togo",
    "flask",
    "comment-x",
    "chevrons-right",
    "book-reader",
    "bible",
    "plane",
    "camera-movie",
    "arrow-from-top",
    "chevron-up-square",
    "bong",
    "badge-dollar",
    "message-edit",
    "key",
    "duplicate",
    "layer",
    "video",
    "camera",
    "dock-right",
    "tag",
    "image",
    "taxi",
    "coupon",
    "conversation",
    "briefcase-alt",
    "phone-outgoing",
    "lock-open",
    "purchase-tag",
    "file-blank",
    "bell-off",
    "file",
    "bot",
    "eyedropper",
    "playlist",
    "pen",
    "archive",
    "ball",
    "package",
    "ruler",
    "heart",
    "folder-plus",
    "map",
    "image-alt",
    "building",
    "battery-charging",
    "film",
    "printer",
    "movie",
    "cog",
    "file-image",
    "sun",
    "book-open",
    "buoy",
    "hide",
    "group",
    "message",
    "book-bookmark",
    "message-square-check",
    "disc",
    "dice-6",
    "arrow-to-right",
    "arrow-from-left",
    "bookmark-heart",
    "car-crash",
    "been-here",
    "microchip",
    "message-x",
    "right-arrow-alt",
    "alarm-exclamation",
    "traffic-cone",
    "message-rounded-check",
    "down-arrow-alt",
    "file-import",
    "message-square-edit",
    "flag-checkered",
    "file-export",
    "bookmark-alt-minus",
    "message-rounded-add",
    "card",
    "calendar-edit",
    "car-wash",
    "arrow-to-bottom",
    "dice-3",
    "message-alt-check",
    "cloud-rain",
    "star-half",
    "microphone-alt",
    "sort-alt",
    "volume-low",
    "chip",
    "discount",
    "checkbox-checked",
    "error-circle",
    "copy",
    "hdd",
    "dock-bottom",
    "eject",
    "map-alt",
    "tag-x",
    "bell-ring",
    "coffee",
    "microphone",
    "first-aid",
    "battery-full",
    "star",
    "cloud-download",
    "alarm-snooze",
    "envelope-open",
    "spray-can",
    "caret-right-square",
    "direction-right",
    "backpack",
    "caret-down-square",
    "webcam",
    "message-add",
    "cctv",
    "megaphone",
    "book-heart",
    "message-alt-edit",
    "up-arrow-alt",
    "tachometer",
    "calendar-week",
    "message-alt-error",
    "blanket",
    "message-minus",
    "message-rounded-edit",
    "calendar-heart",
    "tv",
    "file-pdf",
    "brain",
    "message-alt-add",
    "chevrons-down",
    "badge",
    "home-circle",
    "pizza",
    "brush",
    "chevron-down-square",
    "left-arrow-alt",
    "note",
    "category-alt",
    "bolt",
    "message-square-x",
    "message-rounded-minus",
    "yin-yang",
    "shopping-bags",
    "coin",
    "calendar-exclamation",
    "message-rounded-error",
    "factory",
    "chevron-right",
    "magic-wand",
    "book-add",
    "donate-heart",
    "medal",
    "message-check",
    "badge-check",
    "ship",
    "joystick-button",
    "message-rounded-x",
    "chevron-up-circle",
    "network-chart",
    "florist",
    "mask",
    "chevron-right-square",
    "edit-location",
    "car-garage",
    "help-circle",
    "arrow-to-left",
    "heart-circle",
    "file-archive",
    "crop",
    "message-alt-minus",
    "message-alt-x",
    "arrow-from-bottom",
    "tag-alt",
    "t-shirt",
    "microphone-off",
    "bar-chart-square",
    "dollar-circle",
    "plus-circle",
    "show",
    "x-circle",
    "flag-alt",
    "shopping-bag",
    "tennis-ball",
    "share",
    "phone",
    "alarm",
    "joystick",
    "report",
    "widget",
    "to-top",
    "radio",
    "trash-alt",
    "dock-left",
    "phone-call",
    "bus",
    "calculator",
    "spreadsheet",
    "briefcase",
    "joystick-alt",
    "plus-square",
    "box",
    "hourglass",
    "bug",
    "dislike",
    "edit",
    "window-alt",
    "hotel",
    "chevron-up",
    "landmark",
    "user-account",
    "cart-download",
    "user-badge",
    "calendar-star",
    "right-arrow-square",
    "door-open",
    "down-arrow-square",
    "baseball",
    "heart-square",
    "meteor",
    "magnet",
    "message-square-detail",
    "washer",
    "donate-blood",
    "car-battery",
    "chevron-left-circle",
] as const;

const logoIcons = [
    "kubernetes",
    "dev-to",
    "product-hunt",
    "airbnb",
    "c-plus-plus",
    "markdown",
    "stack-overflow",
    "foursquare",
    "javascript",
    "joomla",
    "snapchat",
    "medium-old",
    "jsfiddle",
    "kickstarter",
    "spring-boot",
    "bootstrap",
    "flickr",
    "wordpress",
    "python",
    "baidu",
    "stripe",
    "microsoft",
    "digg",
    "redux",
    "angular",
    "whatsapp-square",
    "react",
    "django",
    "squarespace",
    "ebay",
    "paypal",
    "windows",
    "bitcoin",
    "zoom",
    "drupal",
    "microsoft-teams",
    "firebase",
    "spotify",
    "deviantart",
    "magento",
    "instagram",
    "medium-square",
    "slack",
    "play-store",
    "mastercard",
    "android",
    "skype",
    "vimeo",
    "blogger",
    "discord",
    "facebook-square",
    "medium",
    "yelp",
    "discourse",
    "internet-explorer",
    "yahoo",
    "firefox",
    "visa",
    "quora",
    "bing",
    "shopify",
    "vk",
    "apple",
    "behance",
    "twitch",
    "edge",
    "opera",
    "unsplash",
    "500px",
    "nodejs",
    "digitalocean",
    "less",
    "periscope",
    "google",
    "git",
    "creative-commons",
    "ok-ru",
    "amazon",
    "blender",
    "soundcloud",
    "codepen",
    "flickr-square",
    "wikipedia",
    "mailchimp",
    "tumblr",
    "messenger",
    "pinterest",
    "dropbox",
    "google-cloud",
    "telegram",
    "tux",
    "vuejs",
    "chrome",
    "trello",
    "wix",
    "sass",
    "invision",
    "html5",
    "css3",
    "dailymotion",
    "linkedin",
    "google-plus-circle",
    "whatsapp",
    "dribbble",
    "facebook",
    "slack-old",
    "linkedin-square",
    "google-plus",
    "twitter",
    "github",
    "youtube",
    "pocket",
    "reddit",
    "mastodon",
    "patreon",
    "upwork",
    "figma",
    "99designs",
    "sketch",
    "docker",
    "discord-alt",
    "aws",
    "gmail",
    "unity",
    "audible",
    "jquery",
    "mongodb",
    "postgresql",
    "tiktok",
    "heroku",
    "venmo",
    "java",
    "algolia",
    "imdb",
    "flutter",
    "go-lang",
    "redbubble",
    "meta",
    "gitlab",
    "steam",
    "pinterest-alt",
    "instagram-alt",
    "flask",
    "etsy",
    "adobe",
    "visual-studio",
    "php",
    "tailwind-css",
    "facebook-circle",
    "trip-advisor",
    "netlify",
    "deezer",
    "xing",
    "graphql",
    "typescript",
] as const;

export type RegularIcon = typeof regularIcons[number];
export type SolidIcon = typeof solidIcons[number];
export type LogoIcon = typeof logoIcons[number];

export type BoxIcon = RegularIcon | SolidIcon | LogoIcon;
